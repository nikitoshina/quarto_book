...
```{r}
# addTitle.R
addTitle <- function(name, title) { # <1>
  return(paste(title, name))
}

myName <- colnames(read.csv("name.csv")) # <2>
```
1. Defines a function called `addTitle`, which concatenates a title and name to create a new string.
2. Read `name.csv` and get the name of the column (name is stored and column name).

```{r}
# sayHi.R
sayHi <- function(name) { # <1>
  return(paste("Hi", name))
}
```
1. Defines a function called `sayHi`, which concatenates a name and "Hi".
...
addTitle(myName, title) |>
  sayHi() |>
  print()
```
1. Named "main.R," loads the `sayHi` and `addTitle` modules from their respective files using the source function.
...
```{r}
# functions/addTitle.R
box::use(
  utils[read = read.csv] # <1>
)

#' @export # <2> exports the function
addTitle <- function(name, title) {
  return(paste(title, name))
}

#' @export
myName <- colnames(read("name.csv"))
```
...
baddTitle$addTitle(baddTitle$myName, title) |> # <3>
  bsayHi$sayHi() |>
  str_split_1(patter = " ") # <4>
```

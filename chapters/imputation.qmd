...
Another good way is to use a premade function from `visdat` called `vismiss`. It will show distribution of missing data, you can prearrange data and facet to better identify missing data.
...
airquality %>%
  arrange(Month, Day) %>%
  mutate(
    imp_fixed = coalesce(Ozone, 0),  # <1> 
    Ozone_missing = is.na(Ozone),
    imp_mean = coalesce(Ozone, round(mean(Ozone, na.rm = TRUE), 2)), # <2> 
    imp_median = coalesce(Ozone, median(Ozone, na.rm = TRUE)), # <3> 
    imp_fill = Ozone,
    .keep = "used"  # <4> 
  ) %>%
...
A simple way to check the effects of the missing data is to replace it with the worst possible outcome and best possible outcome. For instance, if we could impute with Ozone with high values and with low values and compare the results of our analysis. In clinical trials, it is common to control had a beneficial outcome assume mean + 2 standard deviations outcome and treatment group has a harmful outcome mean - 2 standard deviations in the best-worst case and in reverse for worst-best case.
